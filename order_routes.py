...
# Prefix /orders applies to all routes in this router.

# Adds OpenAPI tag "orders" for grouping in Swagger UI.
...
# Binds a SQLAlchemy session to the PostgreSQL engine to query/update the DB.
...
# POST to /orders/order.

# Requires JWT + order body (quantity, pizza size).

# Finds user from JWT.

# Adds new Order to database.

# Returns the new order’s data.
...
async def place_an_order(order:OrderModel,Authorize:AuthJWT=Depends()):
    ...
    # Returns all orders made by the currently logged-in user.

    # Uses relationship in SQLAlchemy (user.orders).
